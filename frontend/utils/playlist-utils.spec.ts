...
describe('parsePlaylistUrl', () => {
    ...
    describe('Spotify URI', () => {
        ...
        it('extracts first 22 chars when URI has ID longer than 22 chars', () => {
            ...
            expect(parsePlaylistUrl('spotify:playlist:37i9dQZF1DXcBWIGoYBM5Mextra')).toBe(
    ...
    describe('web URL', () => {
        ...
        it('extracts ID when URL has query params', () => {
            const id = '37i9dQZF1DXcBWIGoYBM5M';
            expect(parsePlaylistUrl(`https://open.spotify.com/playlist/${id}?si=abc123`)).toBe(id);
        });
    });
    ...
    describe('invalid input', () => {
        ...
        it('returns null for invalid URL format', () => {
            expect(parsePlaylistUrl('https://spotify.com/playlist/37i9dQZF1DXcBWIGoYBM5M')).toBeNull();
            expect(parsePlaylistUrl('https://open.spotify.com/album/37i9dQZF1DXcBWIGoYBM5M')).toBeNull();
        });

        it('returns null for non-Spotify URL', () => {
            expect(parsePlaylistUrl('https://example.com/playlist/37i9dQZF1DXcBWIGoYBM5M')).toBeNull();
        });
    });
});
