...
export function useAdminUpdateStreakQuestion() {
  ...
  return useMutation<StreakQuestionDto, Error, { id: string; dto: UpdateStreakQuestionDto }>({
    mutationFn: async ({ id, dto }) => {
      try {
        return await api.adminControllerUpdateStreakQuestion({ id, updateStreakQuestionDto: dto });
      } catch (e) {
