...
describe('EmailSendLimiter', () => {
  ...
  it('allows the first mail to an address', async () => {
    ...
    await expect(limiter.claim('player@example.com')).resolves.toBe(true);
  });

  it('refuses a second one straight away', async () => {
    const limiter = await build();
    await limiter.claim('player@example.com');
    await expect(limiter.claim('player@example.com')).resolves.toBe(false);
  });

  it('holds separate cooldowns for separate addresses', async () => {
    const limiter = await build();
    await limiter.claim('one@example.com');
    await expect(limiter.claim('two@example.com')).resolves.toBe(true);
  });
