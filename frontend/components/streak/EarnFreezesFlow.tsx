...
export function EarnFreezesFlow({ freezesAvailable: initialFreezes, onClose }: EarnFreezesFlowProps) {
  ...
  const handleNext = useCallback(() => {
    ...
    queryClient.invalidateQueries({ queryKey: queryKeys.streak.quiz });
  }, [queryClient]);
      ...
      <div className="space-y-6 text-center">
        <div className="flex justify-center gap-2">
          {slots.map((filled, i) => (
            ...
            >
              <Snowflake className={`w-5 h-5 ${filled ? "text-cyan-400" : "text-white/20"}`} />
            </motion.div>
          ))}
        </div>
        <div>
          <h3 className="text-lg font-bold text-white">
            {freezeCount >= MAX_FREEZES ? "All slots filled!" : "No more questions!"}
          </h3>
          <p className="text-white/60 text-sm mt-1">
            You have {freezeCount} {freezeCount === 1 ? "freeze" : "freezes"} ready.
          </p>
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1.5">
          {slots.map((filled, i) => (
            ...
            >
              <Snowflake className={`w-3.5 h-3.5 ${filled ? "text-cyan-400" : "text-white/20"}`} />
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Question */}
      <div>
        <span className="text-xs text-white/40 uppercase tracking-wider">{question.category}</span>
      ...
      <div className="space-y-2">
        <AnimatePresence mode="wait">
          {question.options.map((option, idx) => {
            let optionStyle = "border-white/15 hover:border-white/30 bg-white/5";
            ...
            return (
              ...
              >
                <span className="flex items-center gap-3">
                  <span className="text-white/40 font-mono text-xs w-4">{String.fromCharCode(65 + idx)}</span>
                  <span className="text-white">{option}</span>
                  {lastResult && idx === lastResult.correctAnswerIndex && (
                    <Check className="w-4 h-4 text-green-400 ml-auto" />
                  )}
                  {lastResult && idx === selectedAnswer && !lastResult.correct && (
