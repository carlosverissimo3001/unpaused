...
| [**guestSearchControllerSearchTracks**](ApiApi.md#guestsearchcontrollersearchtracks) | **GET** /guest/search/tracks | Search Spotify tracks (guest guess options) |
...
## guestSearchControllerSearchTracks

> Array&lt;TrackOptionDto&gt; guestSearchControllerSearchTracks(q)

Search Spotify tracks (guest guess options)
...
import type { GuestSearchControllerSearchTracksRequest } from '';
...
async function example() {
  ...
  const body = {
    // string
    q: q_example,
  } satisfies GuestSearchControllerSearchTracksRequest;

  try {
    const data = await api.guestSearchControllerSearchTracks(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
