...
## trackGroupControllerBySlug

> TrackGroupDto trackGroupControllerBySlug(slug)
...
import type { TrackGroupControllerBySlugRequest } from '';
...
async function example() {
  ...
  const body = {
    // string
    slug: slug_example,
  } satisfies TrackGroupControllerBySlugRequest;

  try {
    const data = await api.trackGroupControllerBySlug(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
[**TrackGroupDto**](TrackGroupDto.md)
...
async function example() {
  ...
  const body = {
    // 'DECADE' | 'GENRE' | 'FAME' | 'SPECIAL' | Which axis to list. Only DECADE is populated today. (optional)
    type: type_example,
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **type** | `DECADE`, `GENRE`, `FAME`, `SPECIAL` | Which axis to list. Only DECADE is populated today. | [Optional] [Defaults to `undefined`] [Enum: DECADE, GENRE, FAME, SPECIAL] |
