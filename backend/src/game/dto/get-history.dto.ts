...
export class GetHistoryDto {
    @ApiPropertyOptional({ description: "Whether the game is daily", type: Boolean })
    @IsNotNullableOptional()
    @IsBoolean()
    @Transform(({ obj }) => toBoolean(obj.isDaily))
    isDaily?: boolean;

    @ApiPropertyOptional({ description: "The limit of the history", type: Number })
    @IsNotNullableOptional()
    @IsNumber()
    @Min(1)
    @Max(100)
    ...
    @ApiPropertyOptional({ description: "The offset of the history", type: Number })
    @IsNotNullableOptional()
    @IsNumber()
    @Min(0)
    @Transform(({ value }) => Number(value))
